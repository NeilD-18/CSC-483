...
def not_operation(X, n):
    """
    Performs the bitwise NOT operation on an n-bit integer X using cvc5's propositional logic encoding.
    
    Steps:
      1. Convert X to its n-bit binary representation (LSB at index 0).
      2. Declare Boolean variables x0, x1, ..., x(n-1) for the input bits.
      3. Assert that each variable equals its corresponding Boolean value.
      4. Declare Boolean variables r0, r1, ..., r(n-1) for the result bits.
      5. For each i, assert that r_i = NOT x_i.
    ...
    filename = f"not_{X}_{n}.smt2"
    solver, tm, smt_file = create_solver_and_logger(filename)
    
    # Get the Boolean sort and convert integer X to its n-bit binary representation.
    bool_sort = solver.getBooleanSort()
    x_bits = int_to_bits(X, n)
    
    # Declare Boolean variables for each bit of X.
    x_vars = []
    for i in range(n):
        x_var = declareConst(tm, smt_file, bool_sort, f"x{i}")
        x_vars.append(x_var)
    
    # Assert the bit values for X.
    for i in range(n):
    ...
    # Encode the bitwise NOT operation: for each bit, assert r_i = NOT x_i.
    for i in range(n):
        not_term = tm.mkTerm(Kind.NOT, x_vars[i])
        eq_term = tm.mkTerm(Kind.EQUAL, r_vars[i], not_term)
