...
def and_operation(X, Y, n):
    """
    Performs the bitwise AND operation on two n-bit integers X and Y using cvc5's propositional logic encoding.
    ...
    Steps:
      ...
      5. For each i, assert that r_i = x_i AND y_i.
    ...
    filename = f"and_{X}_{Y}_{n}.smt2"
    ...
    # Encode the bitwise AND operation: for each bit, assert r_i = x_i AND y_i.
    for i in range(n):
        and_term = tm.mkTerm(Kind.AND, x_vars[i], y_vars[i])
        eq_term = tm.mkTerm(Kind.EQUAL, r_vars[i], and_term)
